    ...
    async.waterfall(
      [
        ...
        function (userGroups, next) {
          try {
            const hasPublic = permissions.canThis(user.role, 'tickets:public');
            const hasWatcher = ticket.watchers.some(watcher => watcher.email == user.email)
            ...
            if (!groupIds.includes(ticket.group._id.toString())) {
              ...
              if (ticket.group.public && hasPublic || hasWatcher) {
                // Blank to bypass
              } else {
            ...
            if (
              (ticket.owner._id.toString() !== req.user._id.toString() &&
                !permissions.canThis(user.role, 'tickets:viewall')) && !hasWatcher
            ) {
              return res.redirect('/tickets');
            }

            if (!permissions.canThis(user.role, 'comments:view') || !hasWatcher) ticket.comments = [];
