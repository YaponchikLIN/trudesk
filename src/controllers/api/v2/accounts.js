...
accountsApi.saveNotificationToken = async (req, res) => {
  const postData = req.body;
  if (!postData) return apiUtil.sendApiError_InvalidPostData(res);
  if (postData?.sessionUserID) {
    await User.updateOne({ _id: postData.sessionUserID }, { $addToSet: { notificationTokens: postData.currentToken } });
  } else {
    await User.updateOne(
      { accessToken: postData.accessToken },
      { $addToSet: { notificationTokens: postData.currentToken } }
    );
  }
...
accountsApi.saveProfile = async (req, res) => {
  ...
  try {
    ...
    if (!_.isUndefined(payload.chatwootApiKey) && !_.isNull(payload.chatwootApiKey))
      dbUser.chatwootApiKey = payload.chatwootApiKey;
    if (!_.isUndefined(payload.preferences.timezone) && !_.isNull(payload.preferences.timezone))
