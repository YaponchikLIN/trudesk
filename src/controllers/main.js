...
const sendMailEventNewPassword = require('../emitter/events/event_new_password');
...
mainController.loginPost = async function (req, res, next) {
  ...
  if (retrySecs > 0) {
    ...
    Setting.findOne({ name: 'ldapSettings:authorizationDomains' }, (err, setting) => {
      ...
      if (!setting) {
        winston.info('Setting ldapSettings:authorizationDomains not found')
...
mainController.resetPass = function (req, res) {
  ...
  userSchema.getUserByResetHash(hash, function (err, user) {
    ...
    if (now < user.resetPassExpire) {
      ...
      user.save(async function (err, updated) {
        if (err) {
          return res.status(500).send(err.message);
        }
        ...
        const result = sendMailEventNewPassword(data, (result) => {
          if (result == 307) {
