...
const mailerExtension = require('./mailerExtension');
                      ...
                      simpleParser(buffer, async function (err, mail) {
                        ...
                        if (mail.headers.has('from')) {
                          ...
                          if (message.from && message.from != '') {
                            ...
                            if (_.isUndefined(mail.text) && mail?.inReplyTo) {
                            ...
                            if (mail?.cc) {
                              message.watchers = mail.cc.value;
                            } else {
                              message.watchers = [];
                            }
...
function handleMessages(messages, done) {
  ...
  messages.forEach(function (message) {
    ...
    if (
      _.isUndefined(message?.references) &&
      !_.isUndefined(message?.responseToComment) &&
      !_.isEmpty(message?.responseToComment)
    ) {
      userSchema.getUserByEmail(message.from, function (err, user) {
        if (err) winston.warn(err);
        const promise = new Promise((resolve, reject) => {
          mailerExtension.processingWatchers({ message, user }, (err, userDB) => {
            if (err) {
              reject(err);
            } else {
              resolve(userDB);
          ...
          if (!err && user) {
            ...
            if (resultTicketUID) {
              ...
              if (resultTicketUID) {
                ...
                message.responseToComment = null;
                let messagesArray = [];
                messagesArray.push(message);
                handleMessages(messagesArray, done)
            ...
            try {
              comment =
                comment.match(/(.*?)ОТВЕТ-КОММЕНТАРИЙ РАЗМЕЩАЙТЕ ВЫШЕ ЭТОЙ СТРОКИ/gs) ||
                comment.match(/(.*?)Ответ-комментарий размещайте выше этой строки/gs);
              if (comment) {
                comment =
                  comment[0].replace(/ОТВЕТ-КОММЕНТАРИЙ РАЗМЕЩАЙТЕ ВЫШЕ ЭТОЙ СТРОКИ/gi, '') ||
                  comment.match(/(.*?)Ответ-комментарий размещайте выше этой строки/gs);
              } else {
                comment = undefined;
              }
            ...
            Ticket.findOne({ uid: ticketUID }, async function (err, t) {
              ...
              if (message.attachments) {
                const commentId = t.comments.filter((comment) => {
                  return (
                    comment.owner._id == Comment.owner &&
        ...
        async.auto(
          {
            ...
            handleCreateTicket: [
              ...
              async function (results, callback) {
                ...
                //Заполнение наблюдателей
                message.watchers = await Promise.all(message.watchers.map(async email => {
                  return new Promise((resolve, reject) => {
                    userSchema.findOne({ email: email.address }).populate('role').exec(function (err, user) {
                      if (err) {
                        winston.warn(err);
                        reject(err);
                      }
                ...
                message.watchers = message.watchers.filter(user => (typeof user === 'object' && user.role.name !== 'Support' && user.role.name !== 'Admin') || (typeof user === 'string' && user !== message.owner.email)).map(user => {
                  let watcher = {};
                  let chance = new Chance();
                  let key = chance.string({
                    length: 8,
                  ...
                  if (typeof user === 'object') {
                    watcher = { email: user.email, key: key }
                    return watcher;
                  } else {
                    email = user;
                    watcher = { email: email, key: key }
                    return watcher;
                  }
