...
function mailTemplates(callback) {
  ...
  var noteAdded = require('./json/mailer-note-added');
  ...
  var ticketWarning = require('./json/mailer-ticket-warning.json');
  ...
  async.parallel(
    [
      ...
      function (done) {
        templateSchema.findOne({ name: ticketWarning.name }, function (err, templates) {
          if (err) return done(err);
          if (!templates || templates.length < 1) {
            return templateSchema.create(ticketWarning, done);
          }
      ...
      function (done) {
        templateSchema.findOne({ name: userCreated.name }, function (err, templates) {
          if (err) return done(err);
          if (!templates || templates.length < 1) {
            return templateSchema.create(userCreated, done);
          }
      ...
      function (done) {
        templateSchema.findOne({ name: statusChanged.name }, function (err, templates) {
          if (err) return done(err);
          if (!templates || templates.length < 1) {
            return templateSchema.create(statusChanged, done);
          }
      ...
      function (done) {
        templateSchema.findOne({ name: commentAdded.name }, function (err, templates) {
          if (err) return done(err);
          if (!templates || templates.length < 1) {
            return templateSchema.create(commentAdded, done);
          }
      ...
      // function (done) {
      //   templateSchema.findOne({ name: noteAdded.name }, function (err, templates) {
      //     if (err) return done(err);
      //     if (!templates || templates.length < 1) {
      //       return templateSchema.create(commentAdded, done);
      //     }
