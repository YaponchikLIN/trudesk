...
api.watchers = {};
api.watchers.fetch = (payload) => {
  const ticketId = payload.ticketId;
  const limit = payload.limit;
  const skip = payload.skip;
  const excludedIds = payload.excludedIds;
  const sessionUserId = payload.sessionUserId;
  const data = { excludedIds, ticketId, sessionUserId };
  return axios.post(`/api/v2/watchers?limit=${limit}&skip=${skip}`, data).then((res) => {
    return res.data;
  });
};

api.watchers.update = (payload) => {
  return axios.put('/api/v2/watchers', payload).then((res) => {
    return res.data;
  });
};

api.watchers.add = (payload) => {
  return axios.post('/api/v2/watchers', payload).then((res) => {
    return res.data;
  });
};
...
api.tSortings.tSortingUpdated = (payload) => {
  return axios.put('/api/v2/tSortings', payload).then((res) => {
    return res.data;
  });
};
