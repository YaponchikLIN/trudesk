...
import { transferToThirdParty, fetchTicketTypes, setCurrentTicket } from 'actions/tickets';
...
const fetchTicket = (parent) => {
  axios
    .get(`/api/v2/tickets/${parent.props.ticketUid}/${parent.props.sessionUserEmail}`)
    .then((res) => {
...
class SingleTicketContainer extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.socket.on(TICKETS_HISTORY_SET, this.onUpdateHistory);
  ...
  componentWillUnmount() {
    ...
    this.props.socket.off(TICKETS_HISTORY_SET, this.onUpdateHistory);
  ...
  showFirstModal = (showFirstModal, watchers = []) => {
    this.setState({ showFirstModal: showFirstModal });
        ...
        if (res && res.data && res.data.success) {
          ...
          if (newComment) {
            const commentId = this.ticket.comments.filter((comment) => {
              return (
                comment.owner._id == newComment.owner &&
  ...
  handleTabClick = (sectionId) => {
    if (sectionId == 0 || sectionId == 1) sectionId = 0;
    else sectionId = 1;
  ...
  sendNotification() {
    ...
    if (this.getSetting('chatwootSettings')) {
      axios
        .get(`/api/v1/users/${this.ticket.owner.username}`)
        .then((response) => {})
        .catch((error) => {
  ...
  render() {
    this.props.setCurrentTicket(this.ticket?._id);
    console.log('this.ticket.dueDate');
    console.log(this?.ticket?.dueDate);
                    ...
                    <div className="ticket-assignee-wrap uk-clearfix" style={{ paddingRight: 30 }}>
                      ...
                      {hasTicketUpdate && (
                        <AssigneeDropdownPartial
                          ...
                          ticketId={this.ticket._id}
                          ticketAssigneeId={() => {
                            if (this.ticket.assignee._id) this.ticket.assignee._id;
                          }}
                ...
                <div className="page-title-right noshadow">
                  ...
                  <div className="page-top-comments uk-float-right">
                    <a
                      ...
                      onClick={() => {
                        this.props.showModal('SHOW_WATCHERS', {
                          ticketId: this.ticket._id,
                          ownerId: this.ticket.owner._id,
                        });
                      }}
                    >
                      Watchers
                ...
                <div className="page-content-right full-height scrollable">
                  <div className="comments-wrapper">
                    ...
                    {this.hasCommentsOrNotes && (
                      <TruTabWrapper>
                        <TruTabSelectors style={{ marginLeft: 110 }} id="list">
                          <TruTabSelector
                            selectorId={0}
                            label="All"
                            active={true}
                      ...
                      (helpers.canUser('comments:create', true) || helpers.canUser('tickets:notes', true)) && (
                        <div className="uk-width-1-1 ticket-reply uk-clearfix">
                          ...
                          <TruTabWrapper style={{ paddingLeft: 85 }}>
                            <TruTabSelectors showTrack={false} id="post">
...
const mapStateToProps = (state) => ({
  ...
  currentTicket: state.ticketsState.currentTicketId,
