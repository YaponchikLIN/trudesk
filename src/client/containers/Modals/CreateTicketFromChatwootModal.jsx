...
import { TICKETS_UI_ATTACHMENTS_UPDATE } from 'serverSocket/socketEventConsts';
...
class CreateTicketFromChatwootModalContainer extends React.Component {
  ...
  unloadingTheDialog() {
    ...
    axios
      .post('/api/v2/unloadingTheDialog', chatwootPayload)
      .then((response) => {
        ...
        this.messages.map((message) => {
          ...
          if (!message.sender) {
            ...
            senderName = `<p style='color:#39f; font-weight: bold'> ${
              this.clientName
            } <h style="font-size: 11px; color: #545A63; font-weight: lighter;"> ${date.toUTCString()}</h></p>`;
          } else {
            this.agentName = message.sender.name;
            senderName = `<p style='color:#29b955; font-weight: bold'> ${
              this.agentName
            } <h style="font-size: 11px; color: #545A63; font-weight: lighter;"> ${date.toUTCString()} </h></p>`;
          }
  ...
  sendNotification(ticketLink, ticketUID, ticketSubject, ticketStatus) {
    if (this.getSetting('chatwootSettings')) {
      ...
      axios
        .post('/api/v2/sendNotificationChatwoot', chatwootPayload)
        .then((response) => {})
        .catch((error) => {
          console.log(error);
        });
  ...
  propsViewdata() {
    if (this.props.viewdata.size != 0) {
      return this.props.viewdata.get('ticketSettings').get('minSubject');
    } else {
      return 3;
    }
  }
  ...
  render() {
    ...
    return (
      <BaseModal {...this.props} style={{ width: 800 }} options={{ bgclose: false }}>
        <form className={'uk-form-stacked'} onSubmit={(e) => this.onFormSubmit(e)}>
          <div className="uk-margin-medium-bottom">
            ...
            <input
              ...
              data-validation-length={`min${this.propsViewdata()}`}
              data-validation-error-msg={`Please enter a valid Subject. Subject must contain at least ${this.propsViewdata()} characters.`}
            />
          </div>
          <div className="uk-margin-medium-bottom">
            <Grid>
